...
std::string insert_random_code(const std::string& text, int min_bytes, int max_bytes, 
                               std::mt19937& rng, int byte_range) {
...
int main() {
  ...
  for (int i : {1, 4, 16, 32, 64, 128, 256}) {
    std::string input_file = "test.jsonl";
    std::string output_file = "problemset_VSsize" + std::to_string(i) + ".jsonl";
    ...
    while (std::getline(inf, line)) {
      try {
        ...
        if (data.contains("question") && data["question"].is_string()) {
          ...
          processed_data["question"] = insert_random_code(question, 1, 1, rng, i);
        }
